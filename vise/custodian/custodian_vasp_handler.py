...
YK
All the VaspModder must be changed to ObaVaspModder to use ObaIncar class.
"""
...
class DriftErrorHandler(ErrorHandler):
    ...
    def correct(self):
        ...
        # Otherwise set PREC to High so ENAUG can be used to control Augmentation Grid Size
            ...
            actions.append({"dict":   "INCAR",
                            "action": {"_set": {
                                "ENAUG": incar.get("ENCUT", 520) * 2}}})
        # PREC is already high and ENAUG set so just increase it
        else:
            actions.append({"dict":   "INCAR",
                            "action": {"_set": {"ENAUG": int(incar.get("ENAUG",
                                                                       1040) * self.enaug_multiply)}}})

        curr_drift = outcar.data.get("drift", [])[::-1][:self.to_average]
        curr_drift = np.average([np.linalg.norm(d) for d in curr_drift])
        ObaVaspModder(vi=vi).apply_actions(actions)
        return {"errors":  "Excessive drift {} > {}".format(curr_drift,
                                                            self.max_drift),
                "actions": actions}
    ...
    def correct(self):
        ...
        if not v.converged_electronic:
            ...
            if algo == "VeryFast":
                ...
                if not all([v.incar.get(k, "") == val for k, val in
                            new_settings.items()]):
                    actions.append({"dict":   "INCAR",
    ...
    def __init__(self, wall_time=None, buffer_time=300,
        ...
        if "CUSTODIAN_WALLTIME_START" in os.environ:
            self.start_time = datetime.datetime.strptime(
                os.environ["CUSTODIAN_WALLTIME_START"],
                "%a %b %d %H:%M:%S %Z %Y")
        else:
        ...
        if self.wall_time:
            ...
            if not self.electronic_step_stop:
                ...
                outcar.read_pattern({"timings": "LOOP\+.+real time(.+)"},
                                    postprocess=float)
                time_per_step = np.max(
                    outcar.data.get('timings')) if outcar.data.get("timings",
                                                                   []) else 0
            else:
