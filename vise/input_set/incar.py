...
from vise.input_set.input_set import (
    Task, Xc, LDA_OR_GGA, DFT_FUNCTIONAL, HYBRID_FUNCTIONAL, BEYOND_GGA, GW,
    BEYOND_DFT)
...
# This incar_flags should be OrderedDict, but from python 3.6,
# dict uses order-preserving semantics. Furthermore, it does not affect vasp
# result.
...
class ObaIncar(Incar):
    ...
    def from_string(string):
        ...
        NOTE: THIS IS NOT THE SAME AS IT IN THE ORIGINAL INCAR AS SUPPORTING ";"
              TYPE SPLIT.
        ...
        params = {}
        for line in lines:
            # YK: Support the split of ";" notation in INCAR file
            for lline in line.split(";"):
                m = re.match(r'(\w+)\s*=\s*(.*)', lline)
                if m:
                    key = m.group(1).strip()
                    val = m.group(2).strip()
