...
def dos_data():
    ...
    dos_data_w_lims = DosData(xlim=[-100, 100],
                              ylim_set=[[-20, 20], [-30, 30], [-40, 40]],
                              **dos_data_base_args)
    return dos_data, dos_data_w_lims
...
def test_dos_data_energies(dos_data):
    dos_plot_data = dos_data[0].dos_plot_data
    assert dos_plot_data.relative_energies == energies


def test_dos_data_lim(dos_data):
    assert dos_data[0].xlim == [-10, 10]
    assert dos_data[0].ylim_set == [[-5.5, 5.5], [-30.8, 30.8], [-154.0, 154.0]]


def test_dos_data_manual_lim(dos_data):
    dos_plot_data = dos_data[1].dos_plot_data
    assert dos_plot_data.xlim == [-100, 100]
    assert dos_plot_data.ylim_set == [[-20, 20], [-30, 30], [-40, 40]]


def test_dos_data_total_dos(dos_data):
    dos_plot_data = dos_data[0].dos_plot_data
    assert dos_plot_data.doses[0][0].name == "total"
    assert_array_equal(dos_plot_data.doses[0][0].dos, total)


def test_dos_data_pdos_single(dos_data):
    dos_plot_data = dos_data[0].dos_plot_data
    pdos_sum = pdos_list[1] + pdos_list[2]

    assert_array_equal(dos_plot_data.doses[1][0].name, "H-s")
    assert_array_equal(dos_plot_data.doses[1][0].dos, pdos_list[0].s)
    assert_array_equal(dos_plot_data.doses[1][1].name, "H-p")
    assert_array_equal(dos_plot_data.doses[1][1].dos, pdos_list[0].p)

    assert_array_equal(dos_plot_data.doses[2][0].name, "He-s")
    assert_array_equal(dos_plot_data.doses[2][0].dos, pdos_sum.s)
