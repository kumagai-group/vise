...
def test_dos_normal_display(test_data_files: Path):
    with tempfile.TemporaryDirectory() as tmp_dirname:
        dos_vasprun = str(test_data_files / "MgO_dos_vasprun.xml")
        mgo_dos = get_dos_plot(vasprun=dos_vasprun)
        mgo_dos.show()
        mgo_dos.savefig(Path(tmp_dirname) / "MgO_dos.pdf")
